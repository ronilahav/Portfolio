...
public class SokobanView extends Observable implements View, Initializable{
	...
	public void paintLevel(LinkedList<String> arg){
		if(arg.get(1)=="-1")
		{
			newLevelToList(arg.get(0));
		}	
		soundManager(paintManager.paint(arg));//the paint manager return the move number.
	}
	
	private void newLevelToList(String map) {
		currentLevelName = file.getName().substring(0, file.getName().length()-4);
		this.lvlName = currentLevelName;
		dbm.addLevel(currentLevelName, map, 0);
	}
